...
pca_comps <- prcomp(as.data.frame(as.matrix(dense_term_matrix)),tol=0.25)


pcamtx <- as.data.frame(pca_comps$x)

#add the relevance term to data frame
corpus_matrix <- cbind(pcamtx,c(train_file$median_relevance,rep(-1,length(test_content))))

colnames(corpus_matrix)[length(colnames(corpus_matrix))] <- 
  "median_relevance"

#subset training data for model building
train_pca <- corpus_matrix[1:length(train_content),]
test_pca <- corpus_matrix[length(train_content)+1:nrow(corpus_matrix),]
##caret training
...
gbmFit2 <- train(median_relevance~.,data=train_pca,method='gbm',metric="weightedkappa",tuneGrid=gbmGrid,trControl=fitControl,verbose=FALSE)
